...
Le SDK `@smartbills/react-native` fournit des composants mobiles natifs pour afficher des reçus numériques, des codes-barres et des codes QR dans les applications React Native. Il comprend un provider de configuration, des composants de reçus préconstruits et des hooks pour la gestion des reçus.
...
Encapsulez votre application avec `SmartbillsReactNativeProvider`. Il nécessite une instance `SBClient` et un objet de configuration :
...
Le composant `Receipt` affiche un reçu numérique complet avec les informations du commerçant, les articles, les détails de paiement et les totaux :
...
| `ReceiptMerchant` | En-tête avec nom, logo et emplacement du commerçant |
...
| `ReceiptPaymentCash` | Détails de paiement en espèces (montant remis, monnaie) |
...
| `ReceiptOriginal` | Vue de l'image du reçu original numérise |
...
Affiché le logo de la marque d'un commerçant :
...
Formate et affiche une valeur monétaire selon la locale et la devise configurées :
...
## Intégration Caméra pour la Capture de Reçus

Le SDK fonctionne avec les bibliothèques caméra de React Native pour les flux de numérisation de reçus. Utilisez la caméra de l'appareil pour capturer des images de reçus, puis téléchargez-les via le hook `usePresignedUpload` :
