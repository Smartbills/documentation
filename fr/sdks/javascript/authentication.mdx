...
Le SDK Smartbills utilise des jetons Bearer OAuth2 pour l'authentification. Chaque requête inclut le jeton dans l'en-tête `Authorization`, un identifiant d'entreprise dans l'en-tête `x-tenant-id` et une locale dans l'en-tête `Accept-Language`.
...
Pour des scénarios avancés comme le rafraichissement automatique des jetons, implémentez l'interface `CredentialProvider` :
